...
void autonomous(void) {
  ...
  {
    case AWPlineAuton:
      AutonDrive((1/2)*tiles, DRIVE_FAST, DRIVE_MID_ACCEL);  //drive to mogo
      MoGo.setPosition(MOGO_JUNK, degrees); 
      mogoTarg = MOGO_HOLD;  //pick up mogo & drive back 
      AutonTurn(-pi, 14*inches, DRIVE_FAST, DRIVE_MID_ACCEL);  //turn to face line of rings
      ...
      AutonDrive(-(1/2)*tiles, DRIVE_FAST, DRIVE_MID_ACCEL);
      break;
    case NeutralAuton:
      AutonDrive(2*tiles, DRIVE_FAST, DRIVE_MID_ACCEL);  //drive to low neutral goal
      MoGo.setPosition(MOGO_JUNK, degrees);
      ...
      AutonDrive(8*inches, DRIVE_FAST, DRIVE_MID_ACCEL); //go to alliance goal and pick up
      ...
      break;
    case PlatformAuton:
      AutonMogo(MOGO_MID, Msec_TO_sec/2); //grab mogo, score 1 ring in base
      mogoTarg = MOGO_GROUND; //set mogo back down
      AutonTurn(-2*pi/5, WHEEL_TO_WHEEL_DIST, DRIVE_FAST, DRIVE_MID_ACCEL, true);
      AutonDrive(2*tiles + 4*inches, DRIVE_FAST, DRIVE_MID_ACCEL);
      AutonMogo(MOGO_HOLD, Msec_TO_sec/3);  //pick up neutral goal & drive back
      AutonDrive(-1.5*tiles, DRIVE_FAST, DRIVE_MID_ACCEL);
      break;
    case FullAWP:
      AutonMogo(MOGO_MID, Msec_TO_sec/2); //raise mogo, get absolute position, score 1 ring in base
      mogoTarg = MOGO_GROUND; //set mogo back down
      AutonDrive(-1*tiles, DRIVE_FAST, DRIVE_LOW_ACCEL); //bump against wall
      AutonTurn(-pi/2, WHEEL_TO_WHEEL_DIST + 4*inches, DRIVE_FAST, DRIVE_MID_ACCEL); //turn to face other mogo
    ...
    case AWP_Platform:
      ...
      AutonDrive(-(1*tiles - 6*inches), DRIVE_FAST, DRIVE_MID_ACCEL); //bump agains platform
...
void AutonTurn(float radians, int outerRadius, int maxVel, int accel, bool reversed)
{
  ...
  if (radians > GyroStart) //turning right
    while (outerVel >= 0)
    {
      ...
      while (outerVel >= 0) //very similar to right turn
      {
        ...
        GraphDebuggerVars(- radians * rad_TO_deg, - angleLeft * rad_TO_deg); //debugging
  ...
  Drive(0, 0); //stop drive motors to prevent drift
