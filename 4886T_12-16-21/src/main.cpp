...
void autonomous(void) {
  ...
  switch(AutonSelect)
  {
    case AWPlineAuton:
      ...
      AutonTurn(-pi, 12*inches, DRIVE_FAST, DRIVE_MID_ACCEL);  //turn to face line of rings
    ...
    case NeutralAuton:
      ...
      AutonDrive(16*inches, DRIVE_FAST, DRIVE_MID_ACCEL); //go to alliance goal and pick up
    ...
    case PlatformAuton:
      MoGo.setPosition(MOGO_JUNK, degrees);
      mogoTarg = MOGO_HOLD;
      //AutonMogo(MOGO_HOLD, Msec_TO_sec/2); //grab mogo, score 1 ring in base
      AutonDrive(6*inches, DRIVE_FAST, DRIVE_MID_ACCEL);
      AutonTurn(pi, (10*inches + 1.5*tiles)/2, DRIVE_FAST, DRIVE_MID_ACCEL, true);
      Rollers(ROLLER_SPEED);
      AutonDrive(-1*tiles, DRIVE_SLOW, DRIVE_MID_ACCEL);
      AutonMogo(MOGO_GROUND, Msec_TO_sec);
      Rollers(0);
      AutonTurn(5*pi/4, WHEEL_TO_WHEEL_DIST, DRIVE_MID, DRIVE_MID_ACCEL, true);
      AutonDrive(1*tiles, DRIVE_FAST, DRIVE_MID_ACCEL);
      AutonMogo(MOGO_MID, Msec_TO_sec/2);
      AutonTurn(3*pi/2, WHEEL_TO_WHEEL_DIST, DRIVE_MID, DRIVE_MID_ACCEL);
