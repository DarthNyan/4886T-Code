...
int AutonSelect = DebugAuton, AutonSide = RED_SIDE;
...
void autonomous(void) {
  ...
  switch(AutonSelect)
  {
    case AWPlineAuton:
      ...
      AutonTurn(0, WHEEL_TO_WHEEL_DIST - 3*inches, DRIVE_FAST, DRIVE_MID_ACCEL); //face line of rings
    ...
    case SlapRingsAuton:
      ...
      if (FindFrontMogo(NEUTRAL_SIDE, DRIVE_MID) == 0) 
      {
        //AutonDrive(6*inches, DRIVE_FAST, DRIVE_MID_ACCEL);
        OnFrontMogo(); // find and grab goal
      }
      ...
      AutonTurn(-pi/2, WHEEL_TO_WHEEL_DIST/2, DRIVE_FAST, DRIVE_MID_ACCEL); //face alliance goal
    ...
    case PlatformAuton:
      ...
      AutonTurn(2*pi/3, WHEEL_TO_WHEEL_DIST - 1*inches, DRIVE_FAST, 2500);
      wait(1, sec);
      //AutonDrive(-24*inches - StopAndRetDist(), DRIVE_FAST, DRIVE_HIGH_ACCEL);
      OffSlapper();
      Hook.set(false);
    ...
    case CenterRingsAuton:
      ...
      fourTarg = FOUR_UP; //raise lift
      AutonTurn(pi/6, WHEEL_TO_WHEEL_DIST - 3*inches, DRIVE_FAST, DRIVE_MID_ACCEL); //face line of rings
      Rollers(ROLLER_SPEED); //spin rollers
      wait(200, msec);
      AutonDrive(30*inches, DRIVE_MID, DRIVE_MID_ACCEL); //score rings
      AutonDrive(-42*inches, DRIVE_FAST, DRIVE_MID_ACCEL); //bring goal back
    ...
    /*
      ...
      tickSpeed = 5;
      for (int i = 0; i < 400; i += tickSpeed)
      {
        Drive(600, -600);
        GraphDebuggerVars(DriveLB.position(degrees) / 6, GyroAbsPos());
        if(i % 100 == 0) Brain.Screen.drawLine(i/tickSpeed, 0, i/tickSpeed, 200);
        wait(tickSpeed, msec);
      }
      for(int i = 400; i < 1000; i += tickSpeed)
      {
        StopDrive();
        GraphDebuggerVars(DriveLB.position(degrees) / 6, GyroAbsPos());
        if(i % 100 == 0) Brain.Screen.drawLine(i/tickSpeed, 0, i/tickSpeed, 200);
      ...
      AutonTurn(-pi, WHEEL_TO_WHEEL_DIST/2, DRIVE_FAST, DRIVE_HIGH_ACCEL);
      break;
    case SkillsAuton:
      ...
      Rollers(ROLLER_SPEED); //spin intake
      AutonTurn(pi, WHEEL_TO_WHEEL_DIST, DRIVE_FAST, DRIVE_MID_ACCEL); //face star of rings
      AutonDrive(1.2*tiles, DRIVE_MID, DRIVE_MID_ACCEL); //score star of rings
      AutonTurn(pi/2, WHEEL_TO_WHEEL_DIST - 4*inches, DRIVE_FAST, DRIVE_LOW_ACCEL, true); //face platform at angle
      AutonDrive(1*tiles, DRIVE_FAST, DRIVE_LOW_ACCEL, true, true); //drive to platform
      AutonTurn(pi/3, WHEEL_TO_WHEEL_DIST, DRIVE_FAST, DRIVE_MID_ACCEL, true);
      AutonTurn(pi/2, WHEEL_TO_WHEEL_DIST, DRIVE_FAST, DRIVE_MID_ACCEL);
      wait(500, msec); //let goal settle
      OffFrontMogo();
      OnRearMogo();
      ...
      AutonTurn(0, WHEEL_TO_WHEEL_DIST/2, DRIVE_FAST, DRIVE_MID_ACCEL); //turn to face red goal
      FindFrontMogo(AutonSide, DRIVE_MID - 10); //find red goal
      OnFrontMogo();
      AutonDrive(-12*inches, DRIVE_FAST, DRIVE_MID_ACCEL);
      fourTarg = FOUR_UP; //lift red goal
      AutonTurn(pi/2, WHEEL_TO_WHEEL_DIST/2, DRIVE_FAST, DRIVE_LOW_ACCEL, true); //face platform
      AutonDrive(12*inches, DRIVE_FAST, DRIVE_LOW_ACCEL, true, true); //drive to platform
      ...
      AutonDrive(1*tiles + 8*inches, DRIVE_FAST, DRIVE_MID_ACCEL);
      Rollers(ROLLER_SPEED); //spin intake
      AutonTurn(2*pi, WHEEL_TO_WHEEL_DIST, DRIVE_FAST, DRIVE_MID_ACCEL); //face star of rings
      AutonDrive(1.2*tiles, DRIVE_MID, DRIVE_MID_ACCEL); //score rings
      AutonTurn(5*pi/2, WHEEL_TO_WHEEL_DIST/2, DRIVE_FAST, DRIVE_MID_ACCEL, true); //face platform
      ...
      AutonTurn(2*pi, 18*inches, DRIVE_FAST, DRIVE_MID_ACCEL, true);
      fourTarg = FOUR_GROUND;
      Rollers(0);
      AutonDrive(12*inches, DRIVE_FAST, DRIVE_MID_ACCEL); //put a little space between red goal
      AutonTurn(pi , WHEEL_TO_WHEEL_DIST/2, DRIVE_FAST, DRIVE_MID_ACCEL); //face red goal
      FindFrontMogo(AutonSide, DRIVE_MID);
      AutonDrive(8*inches, DRIVE_FAST, DRIVE_MID_ACCEL);
      OnFrontMogo();
      AutonDrive(-28*inches, DRIVE_FAST, DRIVE_MID_ACCEL);
      fourTarg = FOUR_UP;
      AutonTurn(pi/2, WHEEL_TO_WHEEL_DIST/2, DRIVE_FAST, DRIVE_LOW_ACCEL, true); //face platform
      AutonDrive(12*inches, DRIVE_FAST, DRIVE_MID_ACCEL, true, true); //drive to platform
      OffFrontMogo(); //score goal
      AutonTurn(0, WHEEL_TO_WHEEL_DIST, DRIVE_FAST, DRIVE_LOW_ACCEL, true); //Turn
      fourTarg = FOUR_GROUND; //lift down
      AutonDrive(-2*tiles, DRIVE_FAST, DRIVE_MID_ACCEL); //back up
      AutonTurn(pi/6, WHEEL_TO_WHEEL_DIST, DRIVE_FAST, DRIVE_MID_ACCEL); //face left blue goal
      FindFrontMogo(-AutonSide, DRIVE_MID); //grab it
      OnFrontMogo();
      fourTarg = FOUR_HOVER;
      AutonTurn(pi/2, WHEEL_TO_WHEEL_DIST + 4*inches, DRIVE_FAST, DRIVE_MID_ACCEL, true); //turn out of corner
      AutonTurn(0, WHEEL_TO_WHEEL_DIST + 4*inches, DRIVE_FAST, DRIVE_MID_ACCEL, true);
      Rollers(ROLLER_SPEED); //spin rollers
      AutonDrive(3*tiles, DRIVE_FAST, DRIVE_MID_ACCEL); //drive forward, clearing rings out of the way
      AutonTurn(-pi, WHEEL_TO_WHEEL_DIST - 2*inches, DRIVE_FAST, DRIVE_MID_ACCEL, true); //turn around
      FindMogo(-AutonSide, DRIVE_MID - 10); //grab other blue goal
      OffRearMogo();
      fourTarg = FOUR_UP; //raise lift
      AutonTurn(-pi/2, WHEEL_TO_WHEEL_DIST, DRIVE_FAST, DRIVE_MID_ACCEL); //to blue side, scoring rings along the way
      AutonDrive(1*tiles, DRIVE_MID, DRIVE_MID_ACCEL, false);
      AutonDrive(3.5*tiles, DRIVE_FAST, DRIVE_LOW_ACCEL, true, true);
      AutonTurn(-pi, WHEEL_TO_WHEEL_DIST - 3*inches, DRIVE_FAST, DRIVE_MID_ACCEL, true); //face platform
      Rollers(0);
      AutonDrive(12*inches, DRIVE_FAST, DRIVE_LOW_ACCEL, true, true); //bump against platform
      fourTarg = FOUR_GROUND;
      AutonDrive(-12*inches, DRIVE_MID, DRIVE_LOW_ACCEL); //bring platform down
      break;
    default:
      break;
  }
...
void AutonTurn(float radians, int outerRadius, int maxVel, int accel, bool reversed)
{
  ...
  float GYRO_DELAY_TICKS = GYRO_DELAY / tickSpeed, oldGyro = GyroAbsPos() / rad_TO_deg;
  ...
  if (radians > GyroStart) //turning right
    while (outerVel >= 0)
    {
      ...
      angleLeft = radians 
                    -( GyroAbsPos() / rad_TO_deg
                        + GYRO_DELAY_TICKS * (GyroAbsPos()/rad_TO_deg - oldGyro)
                    );
      oldGyro = GyroAbsPos()/rad_TO_deg;
...
void Park()
{
  int oldPitch = Gyro.orientation(roll, degrees), robotPitch, parkD; 
  while (true)
  {
    robotPitch = Gyro.orientation(roll, degrees); //gyro is mounted sideways so its roll is robot's pitch
    parkD = (oldPitch - robotPitch) * PARK_KD;
    if (abs(robotPitch) < LEVEL) StopDrive();
    else Drive(PARK_KP * robotPitch
            + parkD,
          PARK_KP * robotPitch
            + parkD
    );
