...
void autonomous(void) {
  ...
  switch(AutonSelect)
  {
    case AWPlineAuton:
      ...
      AutonMogo(MOGO_MID, Msec_TO_sec/3);  //pick up neutral goal & drive back
      AutonDrive(-1*tiles, DRIVE_FAST, DRIVE_HIGH_ACCEL);
      AutonMogo(MOGO_GROUND, Msec_TO_sec/2);
      AutonDrive(-6*inches, DRIVE_FAST, DRIVE_MID_ACCEL);
      AutonTurn(pi/2, WHEEL_TO_WHEEL_DIST + 3*inches, DRIVE_MID, DRIVE_MID_ACCEL); //turn to face alliance goal.
      AutonDrive(12*inches, DRIVE_FAST, DRIVE_MID_ACCEL); //go to alliance goal and pick up
      MoGo.setPosition(MOGO_JUNK, degrees);
      AutonMogo(MOGO_HOLD, Msec_TO_sec);
      AutonTurn(pi, WHEEL_TO_WHEEL_DIST, DRIVE_MID, DRIVE_MID_ACCEL, true); //face line of rings
      Rollers(ROLLER_SPEED);
      AutonDrive(-1*tiles, DRIVE_MID, DRIVE_MID_ACCEL); //drive over line of rings, scoring them
      AutonDrive(1.5*tiles, DRIVE_FAST, DRIVE_MID_ACCEL); //drive back into zone
    ...
    case NeutralAuton:
      ...
      AutonDrive(-6*inches, DRIVE_FAST, DRIVE_MID_ACCEL);
      AutonTurn(pi/2, WHEEL_TO_WHEEL_DIST + 3*inches, DRIVE_MID, DRIVE_MID_ACCEL); //turn to face alliance goal.
      AutonDrive(6*inches, DRIVE_FAST, DRIVE_MID_ACCEL); //go to alliance goal and pick up
      MoGo.setPosition(MOGO_JUNK, degrees);
      AutonMogo(MOGO_HOLD, Msec_TO_sec);
      ...
      AutonDrive(-0.5*tiles, DRIVE_FAST, DRIVE_MID_ACCEL);
      AutonTurn(0, WHEEL_TO_WHEEL_DIST, DRIVE_MID, DRIVE_MID_ACCEL); //face tall neutral
      AutonDrive(1*tiles, DRIVE_FAST, DRIVE_MID_ACCEL); //go to tall neutral
      AutonMogo(MOGO_MID, Msec_TO_sec/2); //bring it back
    ...
    case PlatformAuton:
      MoGo.setPosition(-160, degrees);
      AutonDrive(45*inches, DRIVE_FAST, DRIVE_HIGH_ACCEL);  //drive to low neutral goal
      AutonMogo(MOGO_MID, Msec_TO_sec/2);  //pick up neutral goal & drive back
      AutonTurn(-pi/2, 1*tiles + WHEEL_TO_WHEEL_DIST/2, DRIVE_FAST, DRIVE_HIGH_ACCEL, true);
    ...
    case FullAWP:
      ...
      AutonTurn(pi/2, WHEEL_TO_WHEEL_DIST, DRIVE_FAST, DRIVE_MID_ACCEL, true); //correct steering incase collision with mogo knocked it off 
      Rollers(ROLLER_SPEED); //roll intake to score preloads
    ...
    case SkillsAuton:
      ...
      AutonTurn(pi, WHEEL_TO_WHEEL_DIST - 3*inches, DRIVE_SLOW, DRIVE_MID_ACCEL); //face rollers toward stars of rings
      AutonDrive(-1.9*tiles, DRIVE_MID, DRIVE_MID_ACCEL); //drive over stars and push them back, score a few
      Rollers(0);
      ...
      AutonTurn(3*pi/2, WHEEL_TO_WHEEL_DIST + 1*inches, DRIVE_FAST, DRIVE_MID_ACCEL, true); //turn to face line of rings
...
#else
      AutonTurn(pi, WHEEL_TO_WHEEL_DIST/2, DRIVE_SLOW, DRIVE_MID_ACCEL); // turn and back up towards wall
      AutonDrive(-2*tiles, DRIVE_FAST, DRIVE_MID_ACCEL);
      AutonTurn(pi/2, 1*tiles, DRIVE_FAST, DRIVE_MID_ACCEL); //face neutral goal
      AutonDrive(6*inches, DRIVE_FAST, DRIVE_MID_ACCEL); //pick up neutral goal
      AutonMogo(MOGO_MID, Msec_TO_sec/2);
      fourTarg = FOUR_UP; //lift up
      AutonTurn(2*pi/3, WHEEL_TO_WHEEL_DIST, DRIVE_MID, DRIVE_MID_ACCEL); //face platform
      AutonDrive(2.3*tiles, DRIVE_FAST, DRIVE_MID_ACCEL); //drive to platform
      mogoTarg = MOGO_GROUND; //deposit goal
      AutonTurn(pi/2, WHEEL_TO_WHEEL_DIST, DRIVE_MID, DRIVE_MID_ACCEL, true);
      AutonDrive(-1*inches, DRIVE_FAST, DRIVE_MID_ACCEL); //back up
      fourTarg = FOUR_GROUND;
      AutonTurn(0, WHEEL_TO_WHEEL_DIST, DRIVE_MID, DRIVE_MID_ACCEL); //face red goal
      AutonDrive(1.5*tiles, DRIVE_FAST, DRIVE_MID_ACCEL); //drive to red goal
      ...
      Rollers(ROLLER_SPEED); //roll intake
      AutonDrive(-3*tiles, DRIVE_MID, DRIVE_MID_ACCEL); //drive over line of rings
...
void AutonDrive(int dist, int maxVel, int accel)
{
  ...
  StopDrive(); //stop the drive at the end to prevent drifting
}
