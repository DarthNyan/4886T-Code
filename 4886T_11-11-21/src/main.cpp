...
void autonomous(void) {
  ...
  switch(AutonSelect)
  {
    case AWPlineAuton:
      ...
      AutonTurn(-pi, 14, DRIVE_FAST, DRIVE_MID_ACCEL);  //turn and bring fourbar up
      Rollers(ROLLER_SPEED);  //roll intake and grab line of rings
      AutonDrive(-40, DRIVE_SLOW, DRIVE_LOW_ACCEL); 
    ...
    case NeutralAuton:
      ...
      AutonDrive(8, DRIVE_FAST, DRIVE_MID_ACCEL);
      AutonMogo(MOGO_GROUND, Msec_TO_sec); //place mogo down
      AutonDrive(-4, DRIVE_FAST, DRIVE_MID_ACCEL);
    ...
    case PlatformAuton:
      ...
      AutonTurn(-2*pi/5, 16, DRIVE_FAST, DRIVE_MID_ACCEL, true);
      AutonDrive(52, DRIVE_FAST, DRIVE_MID_ACCEL);
      AutonMogo(MOGO_HOLD, Msec_TO_sec / 3);  //pick up neutral goal & drive back
      AutonDrive(-36, DRIVE_FAST, DRIVE_MID_ACCEL);
      /*
      AutonDrive(-22, DRIVE_FAST, DRIVE_LOW_ACCEL); //bump against wall
      AutonTurn(-pi/2, 34, DRIVE_FAST, DRIVE_MID_ACCEL);
      AutonDrive(18, DRIVE_FAST, DRIVE_MID_ACCEL);
      MoGo.setPosition(-100, degrees);
      AutonMogo(MOGO_HOLD, Msec_TO_sec);
      AutonDrive(-36, DRIVE_FAST, DRIVE_MID_ACCEL); */
      break;
    case FullAWP:
      ...
      mogoTarg = MOGO_GROUND; //set mogo back down
      AutonDrive(-22, DRIVE_FAST, DRIVE_LOW_ACCEL); //bump against wall
      AutonTurn(-pi/2, 19, DRIVE_FAST, DRIVE_MID_ACCEL);
    ...
    case AWP_Platform:
      mogoTarg = MOGO_MID;
      AutonDrive(96, DRIVE_MID, DRIVE_MID_ACCEL);
      AutonMogo(MOGO_GROUND, Msec_TO_sec);
      AutonDrive(-18, DRIVE_FAST, DRIVE_LOW_ACCEL);
      AutonTurn(-pi/5, 16, DRIVE_FAST, DRIVE_MID_ACCEL);
      AutonDrive(24, DRIVE_FAST, DRIVE_MID_ACCEL);
      MoGo.setPosition(-100, degrees);
      AutonMogo(MOGO_HOLD, Msec_TO_sec * 1.5);
      Rollers(ROLLER_SPEED);
      AutonDrive(-18, DRIVE_MID, DRIVE_LOW_ACCEL);
      AutonMogo(MOGO_GROUND, Msec_TO_sec);
      Rollers(0);
      AutonDrive(-6, DRIVE_MID, DRIVE_LOW_ACCEL);
    default:
      break;
  }
}
...
void AutonTurn(float radians, int outerRadius, int vel, int accel, bool reversed)
{
  ...
  if (radians > GyroStart) //turning right
    while (outerVel >= 0)
    {
      ...
      if(radians <= GyroAbsPos() / rad_TO_deg) break; //turning uses input from two different sensors. If gyro says position is reached, trust it over encoders
